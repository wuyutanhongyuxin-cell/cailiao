...
| 阶段 2B 证据包校验器 | 收尾 v1：`docs/STAGE2B_EVIDENCE_PACKAGE_VALIDATOR.md` + 模板占位 `examples/stage2b_evidence_package.example.json` + 确定性 `build_stage2b_evidence_package_validator(config=None)` 与 CLI `tools/check_stage2b_evidence_package_validator.py`；复用既有 audit 与七个 Stage 2B 契约/状态 helper（artifact contracts / eval-run contract / observability / release dossier / reproducibility / risk register / industry checklist）为单一事实源,把全部外部 blocker 证据聚合为一处就绪报告:固定 7 个证据组（declared_artifacts、eval_run_manifest、observability_snapshot、release_dossier、reproducibility_provenance、risk_treatment、industry_checklist）、5 个 blocker（行 97/100/103/107/114）、逐 blocker×组的所需证据摘要,并报告各组当前就绪度;默认 `ready_for_stage2b_completion=false`、CLI 退出 1、`roadmap_parent_items_checked=false`。边界:仅元数据/证据包校验,不联网/不调用 provider/不下载模型/不运行评测/不读工件文件或哈希/不读 .env/凭据/不伪造 approval 或 risk acceptance,不勾选任何真实父项 |
| 阶段 2B 公共真实查询集接入（DoIT，候选证据） | 收尾 v1：`docs/STAGE2B_REAL_QUERY_PUBLIC_DOIT_INTAKE.md` + 发明性 fixture `tests/data/doit_fixture.sample.jsonl` + 纯标准库工具 `tools/prepare_stage2b_real_query_set.py`（`prepare`/`validate` 两个子命令）；为 ROADMAP 行 97 提供**公共许可数据**（Hugging Face `ChiyuSONG/dynamics-of-instruction-tuning`，MIT）而非合成占位的真实查询**候选**路径：仅处理本地已下载的 DoIT 记录，从 `Creative Writing` 类别选 50-100 条中文用户 prompt，去重并拒绝过短/空/非中文，写出含 source URL/license/extraction_method/record_count/逐 prompt sha256、**不含助手答案**的确定性产物；`validate` 在数量/许可/来源/schema/内容校验失败时非零退出。工具从不联网、不读 secret、不伪造记录。**本环境 WSL 无出网/DNS，Claude 未在 WSL 产出真实 DoIT 产物；随后 Codex 在 networked Windows sidecar 下载 MIT 许可源文件并生成 `docs/evidence/stage2b/doit_creative_writing_real_query_candidate_100.json`（100 条，候选证据）。边界：候选证据非私有生产遥测、非合成占位；获得候选集不等于完成行 97，五个真实父项保持未勾选。** |
| 阶段 2B DoIT→ready_real BM25 数据集 | 收尾 v1：`docs/STAGE2B_DOIT_BM25_DATASET.md` + 发明性 fixture `tests/data/doit_bm25_fixture.sample.jsonl` + 纯标准库工具 `tools/build_stage2b_doit_bm25_dataset.py`（`build`/`validate`）；把本地已下载的 DoIT prompt-answer 记录转成带**配对语料**的 `ready_real` 查询集，供既有受门禁 BM25 扫参使用：每条选中的 Creative Writing 记录（`creative_writing`/`Creative Writing` 别名均接受）用户 prompt 作 case `query`、助手答案作**公共 MIT 语料文档**，case 的 `relevant_titles` 指向配对文档；case 不含任何助手答案文本。校验委托 `summarize_real_query_readiness`（须 `ready_real`）+ 语料配对 + set_hash 完整性；`build` 选不满 `--min` 即非零退出、不伪造。**本环境 WSL 无出网，未下载真实 DoIT、未产出真实数据集；仅落地工具/文档/测试（发明性 fixture 上验证 `ready_real` 且既有 BM25 扫参可运行）。边界：真实公共基准种子（MIT），非私有生产遥测、非最终生产校准；不勾选任何真实父项，五个 blocker 保持未勾选，由 Codex 在真实文件上决定后续。** |
...
| 最小任务 UI | 收尾 v1：前端新增「任务」面板，仅调用同源后端任务 API，把「新建/列出/载入/保存任务 → 附加当前起草与本地证据 → 任务证据检索/附加/批准 → 生成/硬审/导出预检/导出 DOCX」串成可操作闭环。用起草区要素/事实/草稿/本地证据 `POST /api/tasks` 新建，`GET /api/tasks` 刷新列表，载入任务回填起草区与 `state.evidence`，`PUT /api/tasks/{id}` 回存，任务证据走 `POST /api/tasks/{id}/evidence/search|attach|approve`，`POST /api/tasks/{id}/generate|audit|export/preflight|export/docx` 完成生成审稿与导出。新增独立 GET helper（与既有 POST `api` 分离），所有服务端/用户文本经 `escapeHtml`，未选任务时给简明状态而非报错。任务证据批准明确标注为人工/确定性的词面确认，不做语义蕴含或真伪判断。边界：仅最小可操作面板，不重写应用、不删既有面板、不改后端路由、不加依赖、不联网（除浏览器→本地后端）；证据逐条勾选批准、任务列表分页/搜索等仍待后续 |

## 最终收尾方案

项目后续应优先把现有能力收束为端到端材料任务闭环，而不是继续扩展分散的评测/治理骨架。详细实施清单见 [docs/FINAL_DELIVERY_PLAN.md](docs/FINAL_DELIVERY_PLAN.md)。

收尾进度：MaterialTask v1 后端任务状态与 API 已接入，昨晚形成的分析、状态机、段落版本、锁定、批准事实等能力现在可挂到同一任务记录上。前端最小任务 UI v1 已落地，可在浏览器里跑通「新建/载入/保存 → 任务证据检索/附加/批准 → 生成/硬审/预检/导出」的同源闭环。证据逐条勾选批准、任务列表分页/搜索与更完整的前端编排仍是下一阶段；Stage 2B 五个真实外部父项未受影响，保持未勾选。

收尾进度：任务级证据 API v1 已接入，支持按任务搜索资料库、附加证据、批准证据并提升为 `approved_facts`，再由任务级分析读取这些已批准事实。当前推荐仍是确定性资料库检索，不是语义 provider。

收尾进度：任务级生成/审计/导出 API v1 已接入，支持 `generate`、`audit`、DOCX `preflight` 和任务 DOCX 导出，并把草稿版本、最新分析和导出预检元数据回写到任务记录。边界：不新增模型 provider，不自动批准，不做前端。
