...
- 当前阶段：`MVP + 阶段 1 完成 + 阶段 2A 完成 + 阶段 2B 检索评测基座已启动`
- 当前重点：阶段 2B 混合检索与引用验证深化（真实匿名查询集、中文 BM25/FTS 调优、向量检索、重排、引用蕴含与冲突检测）、阶段 3 逐段修复闭环
...
- 阶段 2B（BM25/FTS v1）已加入确定性 `bm25_like` 通道（IDF + tf 饱和 + 文档长度归一，1-4 字中文 ngram），与词面/ngram 通道 RRF 融合；但 `k1`/`b` 仍为默认值、尚未在更大真实查询集上扫参校准，也仍无向量库、embedding 与重排模型。
- 主张核验为保守词面规则：阶段 2A 判断文号/年份/数字/政策标记是否被证据覆盖，阶段 2B 进一步把每个标记精确归因到覆盖它的分段并给出覆盖率与逐分段命中详情；但这仍是**词面覆盖**，不等于语义蕴含——尚无真正语义蕴含、冲突证据检测和跨句推理。
