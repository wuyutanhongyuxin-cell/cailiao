...
> 当前版本为可运行的早期版本（MVP + 阶段 1A）。它已经具备材料录入、缺项阻断、证据台账、确定性审稿、模型调用、DOCX 导出，以及可信资料库的文档导入、去重、分段与引用状态管理；混合检索与引用验证、逐段修复和国标级 Word 排版仍在路线图中。
...
- 当前阶段：`MVP + 阶段 1A 可信资料库`
- 当前重点：完善资料库（阶段 1B）、混合检索与引用溯源、逐段修复闭环
...
- 当前证据匹配是规则与字符串启发式，尚无语义检索、BM25/向量与 RRF 融合，也无引用蕴含验证。
- 资料库仅支持 TXT/HTML/DOCX 导入；PDF/XLSX/DOC 一律隔离为不支持格式，不会静默入库。
- DOCX 抽取仅取 OOXML 正文段落文本（w:t），不含表格、文本框、页眉页脚等结构。
- 尚未建立权威等级排序、政策版本链与废止推导，也没有按来源 URL/版本的增量更新，去重仅按内容 SHA256。
- 证据快照与定位为基础实现（规范化正文的字符偏移），尚无更丰富的原文快照与精确定位。
