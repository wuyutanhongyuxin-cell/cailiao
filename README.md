...
> 当前版本为可运行的早期版本（MVP + 阶段 1 可信资料库已完成）。它已经具备材料录入、缺项阻断、证据台账、确定性审稿、模型调用、DOCX 导出，以及可信资料库的文档导入、去重、分段与引用状态、权威分级、版本/取代关系、同源新版本处理和 XLSX 基础导入；混合检索与引用验证、逐段修复和国标级 Word 排版仍在路线图中。
...
- 当前阶段：`MVP + 阶段 1 可信资料库已完成`
- 当前重点：阶段 2 混合检索与引用验证（BM25 + 向量 + RRF、引用蕴含与冲突检测）、逐段修复闭环
...
- 尚无混合检索：没有中文 BM25、向量检索与 RRF 融合，证据匹配仍是规则与字符串启发式（阶段 2）。
- 尚无引用蕴含与冲突证据验证：主张到证据的精确映射与蕴含/冲突检测仍在路线图（阶段 2）。
