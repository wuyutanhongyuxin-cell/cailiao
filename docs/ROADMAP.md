...
阶段 1 拆分为已完成的 1A（文档导入与台账基础）和待办的 1B（权威分级、版本治理与更丰富快照）。整体阶段 1 尚未完成。
...
- [x] PDF/XLSX/DOC 显式隔离为不支持格式，解析失败与隔离记录可查询，不静默丢失
...
- [ ] 管理政策版本链、废止和失效状态推导
- [ ] 按来源 URL/版本的增量更新（当前仅按内容 SHA256 去重）
- [ ] 更丰富的证据原文快照与精确定位（当前为规范化正文的字符偏移）

阶段 1B 验收：每个引用可回到原始文件、URL 和原文；同源新版本可增量更新而非重复；失效政策不默认推荐；权威等级可用于排序与过滤。

验证说明：Codex 在 WSL 与 Windows 发布仓库分别运行 `python -m unittest discover -s tests -v`，19 项测试通过（19 tests OK）。
...
2. 完善资料库（阶段 1B）：权威分级、版本链与增量更新。
3. 建立 50 至 100 个真实检索问题的匿名评测集。
4. 接入 BM25 + 向量 + RRF 和引用定位。
5. 实现段落级状态机与定点修复。
