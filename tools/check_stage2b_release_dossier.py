#!/usr/bin/env python3
"""Report Stage 2B release dossier contract."""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="check_stage2b_release_dossier",
        description="Report Stage 2B release dossier / model-data card contract.")
    ...
    report = _load_server().build_stage2b_release_dossier(config)
    if args.json:
        print(json.dumps(report, ensure_ascii=False, indent=2))
    else:
        print(f"entries: {report['entry_count']}  missing: {len(report['missing_release_evidence_ids'])}  "
              f"ready: {report['ready_for_release']}")
